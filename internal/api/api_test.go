...
// which is what dates the repo. listRepos only stats, so no real git repo is needed.
...
func TestRepoActivityDateGitlink(t *testing.T) {
	root := t.TempDir()
	...
	if got := repoActivityDate(dir); got != "2024-03-04" {
		t.Errorf("repoActivityDate = %q, want 2024-03-04", got)
	}
}
